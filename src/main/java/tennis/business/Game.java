...
public class Game {
	...
	private int playerWhoWinOnePoint;
	...
	public int getPlayerWhoWinOnePoint() {
		return this.playerWhoWinOnePoint;
	}

	public void setPlayerWhoWinOnePoint(int playerWhoWinOnePoint) {
		this.playerWhoWinOnePoint = playerWhoWinOnePoint;
	}
	 ...
	 */
	public void chooseRandomPlayerToWinOnePoint(Random r) {
		if (!isGameOver) {
			this.playerWhoWinOnePoint = r.nextInt(2) + 1;
			switch (this.playerWhoWinOnePoint) {
			case 1:
	 ...
	 */
	public String displayScoreMainMessage() {
	...
	private String displayGameWinnerMessage() {
		...
		if (!isGameOver) {
			return displayGameOneWinMessage(sb);
		} else {
			return displayMatchWinnerMessage(sb);
		}
	}
	 ...
	 * @return the game winner message to display
	 */
	private String displayGameOneWinMessage(StringBuilder sb) {
		if (!isTieBreakRule) {
			if (this.playerWhoWinOnePoint == 1) {
	...
	/**
	 * Build the message with the winner of the match with or without TIE-BREAK RULE
	 * ACTIVATED
	 * 
	 * @return the game winner message to display
	 */
	private String displayMatchWinnerMessage(StringBuilder sb) {
		if (this.player1.getSet() > this.player2.getSet() || this.player1.getTieBreak() > this.player2.getTieBreak()) {
	...
	private String displayScoreMessage() {
		// formatting the message to display
		StringBuilder sb = new StringBuilder(EMPTY_STRING);
		if (this.playerWhoWinOnePoint > 0) {
	...
	private String formatGameWinnerMessage(StringBuilder sb) {
		if (!isTieBreakRule) {
			// try-with-resources statement throws RuntimeException
			try (Formatter fmt = new Formatter(sb)) {
				fmt.format(GAME_END_FORMATTER_PATTERN, this.playerWhoWinOnePoint, this.player1.getName(),
			...
			// try-with-resources statement throws RuntimeException
			try (Formatter fmt = new Formatter(sb)) {
				fmt.format(TIE_BREAK_END_FORMATTER_PATTERN, this.playerWhoWinOnePoint, this.player1.getName(),
						this.player1.displayScorePoints(), this.player2.getName(), this.player2.displayScorePoints(),
						this.player1.getName(), this.player1.displayScoreSets(), this.player2.getName(),
	...
	private String formatScoreMessage(StringBuilder sb) {
		// try-with-resources statement throws RuntimeException
		try (Formatter fmt = new Formatter(sb)) {
			fmt.format(POINT_SCORE_FORMATTER_PATTERN, this.playerWhoWinOnePoint, this.player1.getName(),
					this.player1.displayScorePoints(), this.player2.getName(), this.player2.displayScorePoints());
		}
		return sb.toString();
	}
	...
	private String formatScoreWithDeuceMessage(StringBuilder sb) {
		// try-with-resources statement throws RuntimeException
		try (Formatter fmt = new Formatter(sb)) {
			fmt.format(POINT_SCORE_FORMATTER_PATTERN, this.playerWhoWinOnePoint, this.player1.getName(), DEUCE,
	...
	private String formatScoreWithAdvantageMessage(StringBuilder sb) {
		...
		try (Formatter fmt = new Formatter(sb)) {
			...
			if (this.playerWhoWinOnePoint == 1) {
				fmt.format(POINT_SCORE_FORMATTER_PATTERN, this.playerWhoWinOnePoint, this.player1.getName(), ADVANTAGE,
						this.player2.getName(), scoreFourty);
			} else if (this.playerWhoWinOnePoint == 2) {
				fmt.format(POINT_SCORE_FORMATTER_PATTERN, this.playerWhoWinOnePoint, this.player1.getName(),
						scoreFourty, this.player2.getName(), ADVANTAGE);
			}
		}
		return sb.toString();
	}
